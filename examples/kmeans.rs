...
/// Extracts a palette from an image using the k-means algorithm.
fn main() {
    ...
    let Ok(palette) = Palette::<f32>::extract_with_algorithm(&image_data, Algorithm::KMeans) else {
        eprintln!("Failed to extract with k-means algorithm from image data");
        return;
    };
