...
import { type Algorithm, Color, Palette, Swatch, type Theme } from '@auto-palette/core';
...
describe('@auto-palette/wasm/palette', () => {
  describe('constructor', () => {
    it('should create a Palette instance', () => {
      ...
      const actual = new Palette([
        new Swatch(Color.fromHexString('#6DE1D2'), { x: 120, y: 240 }, 100, 0.25),
        new Swatch(Color.fromHexString('#FFD63A'), { x: 200, y: 300 }, 100, 0.25),
        new Swatch(Color.fromHexString('#FFA955'), { x: 150, y: 100 }, 100, 0.25),
        new Swatch(Color.fromHexString('#FF6F61'), { x: 300, y: 400 }, 100, 0.25),
      ]);
  ...
  describe('findSwatches', () => {
    ...
    beforeAll(() => {
      palette = new Palette([
        new Swatch(Color.fromHexString('#FF6F61'), { x: 120, y: 240 }, 150, 0.15),
        new Swatch(Color.fromHexString('#6DE1D2'), { x: 200, y: 300 }, 360, 0.36),
        new Swatch(Color.fromHexString('#FFD63A'), { x: 150, y: 100 }, 300, 0.3),
        new Swatch(Color.fromHexString('#3F4F44'), { x: 300, y: 400 }, 80, 0.08),
        new Swatch(Color.fromHexString('#A27B5C'), { x: 400, y: 500 }, 60, 0.06),
        new Swatch(Color.fromHexString('#604652'), { x: 500, y: 600 }, 40, 0.04),
        new Swatch(Color.fromHexString('#F7CFD8'), { x: 120, y: 240 }, 30, 0.03),
        new Swatch(Color.fromHexString('#F4F8D3'), { x: 200, y: 300 }, 30, 0.03),
        new Swatch(Color.fromHexString('#A6D6D6'), { x: 150, y: 100 }, 20, 0.02),
        new Swatch(Color.fromHexString('#210F37'), { x: 300, y: 400 }, 20, 0.02),
        new Swatch(Color.fromHexString('#2A0A1D'), { x: 400, y: 500 }, 10, 0.01),
        new Swatch(Color.fromHexString('#4F1C51'), { x: 500, y: 600 }, 10, 0.01),
      ]);
    });
    ...
    ])('should find the swatches from the palette with $theme theme', ({ theme, count, expected }) => {
      // Act
  ...
  describe('extract', { timeout: 10_000 }, () => {
    ...
    it.each([
      ...
      { algorithm: 'slic' },
      { algorithm: 'snic' },
    ])('should extract a palette from an image with the $algorithm algorithm', ({ algorithm }) => {
      // Act
